...
export function PurchaseStatusBadge({ status }: Readonly<{ status: PurchaseStatus | string }>) {
  const className = getClassName(status)
  const label = status === 'Received' ? 'Recibida' : status === 'Cancelled' ? 'Cancelada' : 'Borrador'

  return <span className={className}>{label}</span>
}

function getClassName(status: PurchaseStatus | string) {
  if (status === 'Received') {
