...
public sealed class SaleReturnItemConfiguration : IEntityTypeConfiguration<SaleReturnItem>
{
  public void Configure(EntityTypeBuilder<SaleReturnItem> builder)
  {
    ...
    builder.ToTable("sale_return_items", "sales");
    ...
    builder.Property(item => item.UnitCost).HasPrecision(18, 2).IsRequired(false);
    builder.Ignore(item => item.LineTotal);
    builder.HasIndex(item => new { item.SaleReturnId, item.SaleItemId });
